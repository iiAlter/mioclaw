 ...
 * `mioclaw update`. Use it to verify the completion installation flow
...
\`mioclaw update\`, \`mioclaw doctor\`, and \`mioclaw onboard\`.
...
async function main() {
  ...
  if (status.profileInstalled && status.cacheExists && !options.force) {
    ...
    console.log(
      theme.muted("  1. Remove the '# Mioclaw Completion' block from your shell profile"),
    );
  ...
  if (isCancel(shouldInstall) || !shouldInstall) {
    console.log(theme.muted(`Skipped. Run \`mioclaw completion --install\` later to enable.`));
    return;
  }
