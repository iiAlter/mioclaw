...
describe("control UI routing", () => {
  ...
  it("infers nested base paths", async () => {
    const app = mountApp("/apps/mioclaw/cron");
    await app.updateComplete;

    expect(app.basePath).toBe("/apps/mioclaw");
    expect(app.tab).toBe("cron");
    expect(window.location.pathname).toBe("/apps/mioclaw/cron");
  });

  it("honors explicit base path overrides", async () => {
    window.__OPENCLAW_CONTROL_UI_BASE_PATH__ = "/mioclaw";
    const app = mountApp("/mioclaw/sessions");
    await app.updateComplete;

    expect(app.basePath).toBe("/mioclaw");
    expect(app.tab).toBe("sessions");
    expect(window.location.pathname).toBe("/mioclaw/sessions");
  });
  ...
  it("strips query token params without importing them", async () => {
    ...
    expect(JSON.parse(localStorage.getItem("mioclaw.control.settings.v1") ?? "{}").token).toBe(
      undefined,
    );
  ...
  it("hydrates token from URL hash when settings already set", async () => {
    localStorage.setItem(
      "mioclaw.control.settings.v1",
      JSON.stringify({ token: "existing-token", gatewayUrl: "wss://gateway.example/mioclaw" }),
    );
    const app = mountApp("/ui/overview#token=abc123");
    await app.updateComplete;

    expect(app.settings.token).toBe("abc123");
    expect(JSON.parse(localStorage.getItem("mioclaw.control.settings.v1") ?? "{}")).toMatchObject({
      gatewayUrl: "wss://gateway.example/mioclaw",
    });
    expect(JSON.parse(localStorage.getItem("mioclaw.control.settings.v1") ?? "{}").token).toBe(
      undefined,
    );
  ...
  it("clears the current token when the gateway URL changes", async () => {
    ...
    gatewayUrlInput!.value = "wss://other-gateway.example/mioclaw";
    gatewayUrlInput!.dispatchEvent(new Event("input", { bubbles: true }));
    await app.updateComplete;

    expect(app.settings.gatewayUrl).toBe("wss://other-gateway.example/mioclaw");
    expect(app.settings.token).toBe("");
  });

  it("keeps a hash token pending until the gateway URL change is confirmed", async () => {
    const app = mountApp(
      "/ui/overview?gatewayUrl=wss://other-gateway.example/mioclaw#token=abc123",
    );
    await app.updateComplete;

    expect(app.settings.gatewayUrl).not.toBe("wss://other-gateway.example/mioclaw");
    expect(app.settings.token).toBe("");
