...
enum MioclawLogging {
    ...
    private static let didBootstrap: Void = {
        LoggingSystem.bootstrap { label in
            ...
            let osHandler = MioclawOSLogHandler(subsystem: subsystem, category: category)
            let fileHandler = MioclawFileLogHandler(label: label)
...
extension Logging.Logger {
    init(subsystem: String, category: String) {
        MioclawLogging.bootstrapIfNeeded()
        let label = MioclawLogging.makeLabel(subsystem: subsystem, category: category)
...
struct MioclawOSLogHandler: AppLogLevelBackedHandler {
...
struct MioclawFileLogHandler: AppLogLevelBackedHandler {
    let label: String
    var metadata: Logger.Metadata = [:]
        ...
        line: UInt)
    {
        ...
        let (subsystem, category) = MioclawLogging.parseLabel(self.label)
