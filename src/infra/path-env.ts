...
function candidateBinDirs(opts: EnsureOpenClawPathOpts): { prepend: string[]; append: string[] } {
  ...
  // Bundled macOS app: `mioclaw` lives next to the executable (process.execPath).
  try {
    const execDir = path.dirname(execPath);
    const siblingCli = path.join(execDir, "mioclaw");
  ...
  if (allowProjectLocalBin) {
    ...
    if (isExecutable(path.join(localBinDir, "mioclaw"))) {
...
/**
 * Best-effort PATH bootstrap so skills that require the `mioclaw` CLI can run
