...
const DEFAULT_PACKAGE_NAME = "mioclaw";
...
function normalizeTag(tag?: string) {
  return normalizePackageTagInput(tag, ["mioclaw", DEFAULT_PACKAGE_NAME]) ?? "latest";
}
...
export async function runGatewayUpdate(opts: UpdateRunnerOptions = {}): Promise<UpdateRunResult> {
  ...
  if (gitRoot && pkgRoot && path.resolve(gitRoot) === path.resolve(pkgRoot)) {
    ...
    if (channel === "dev") {
      ...
      const preflightRoot = await fs.mkdtemp(path.join(os.tmpdir(), "mioclaw-update-preflight-"));
    ...
    const doctorEntry = path.join(gitRoot, "mioclaw.mjs");
    ...
    const doctorStep = await runStep(
      step("mioclaw doctor", doctorArgv, gitRoot, { OPENCLAW_UPDATE_IN_PROGRESS: "1" }),
    );
