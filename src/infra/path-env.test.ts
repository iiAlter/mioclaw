...
describe("ensureOpenClawCliOnPath", () => {
  ...
  it("prepends the bundled app bin dir when a sibling mioclaw exists", () => {
    const tmp = abs("/tmp/mioclaw-path/case-bundled");
    const appBinDir = path.join(tmp, "AppBin");
    const cliPath = path.join(appBinDir, "mioclaw");
    setDir(tmp);
    setDir(appBinDir);
    setExe(cliPath);
  ...
  it("prepends mise shims when available", () => {
    const tmp = abs("/tmp/mioclaw-path/case-mise");
    const appBinDir = path.join(tmp, "AppBin");
    const appCli = path.join(appBinDir, "mioclaw");
    setDir(tmp);
    setDir(appBinDir);
    setExe(appCli);
  ...
  it("only appends project-local node_modules/.bin when explicitly enabled", () => {
    const tmp = abs("/tmp/mioclaw-path/case-project-local");
    const appBinDir = path.join(tmp, "AppBin");
    const appCli = path.join(appBinDir, "mioclaw");
    setDir(tmp);
    setDir(appBinDir);
    ...
    const localCli = path.join(localBinDir, "mioclaw");
  ...
  it("prepends Linuxbrew dirs when present", () => {
    const tmp = abs("/tmp/mioclaw-path/case-linuxbrew");
