...
describe("resolveEffectiveHomeDir", () => {
  it("prefers OPENCLAW_HOME over HOME and USERPROFILE", () => {
    ...
    expect(resolveEffectiveHomeDir(env, () => "/fallback")).toBe(path.resolve("/srv/mioclaw-home"));
  });
...
describe("expandHomePrefix", () => {
  it("expands tilde using effective home", () => {
    const value = expandHomePrefix("~/x", {
      env: { OPENCLAW_HOME: "/srv/mioclaw-home" } as NodeJS.ProcessEnv,
    });
    expect(value).toBe(`${path.resolve("/srv/mioclaw-home")}/x`);
  });
