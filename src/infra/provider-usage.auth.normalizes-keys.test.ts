...
describe("resolveProviderAuths key normalization", () => {
  ...
  beforeAll(async () => {
    suiteRoot = await fs.mkdtemp(path.join(os.tmpdir(), "mioclaw-provider-auth-suite-"));
  });
  ...
  async function withSuiteHome<T>(
    ...
    await fs.mkdir(path.join(base, ".mioclaw", "agents", "main", "sessions"), { recursive: true });
    ...
    process.env.OPENCLAW_STATE_DIR = path.join(base, ".mioclaw");
  ...
  async function writeAuthProfiles(home: string, profiles: Record<string, unknown>) {
    const agentDir = path.join(home, ".mioclaw", "agents", "main", "agent");
