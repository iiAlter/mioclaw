...
async function withGlobalControlUiHardlinkFixture<T>(run: (rootPath: string) => Promise<T>) {
  ...
  try {
    const packageRoot = path.join(tmp, "pnpm-global", "5", "node_modules", "mioclaw");
    const controlUiRoot = path.join(packageRoot, "dist", "control-ui");
    await fs.mkdir(controlUiRoot, { recursive: true });
    await fs.writeFile(path.join(packageRoot, "package.json"), JSON.stringify({ name: "mioclaw" }));
...
describe("gateway.controlUi.root", () => {
  test("rejects hardlinked index.html when configured root points at global Mioclaw package control-ui", async () => {
