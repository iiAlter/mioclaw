...
const serviceReadCommand = vi.fn(async (_env?: NodeJS.ProcessEnv) => ({
  ...
  environment: {
    OPENCLAW_STATE_DIR: "/tmp/mioclaw-daemon",
    OPENCLAW_CONFIG_PATH: "/tmp/mioclaw-daemon/mioclaw.json",
  },
}));
...
const resolveStateDir = vi.fn(
  (env: NodeJS.ProcessEnv) => env.OPENCLAW_STATE_DIR ?? "/tmp/mioclaw-cli",
);
const resolveConfigPath = vi.fn((env: NodeJS.ProcessEnv, stateDir: string) => {
  return env.OPENCLAW_CONFIG_PATH ?? `${stateDir}/mioclaw.json`;
});
...
vi.mock("../../config/config.js", () => ({
  createConfigIO: ({ configPath }: { configPath: string }) => {
    const isDaemon = configPath.includes("/mioclaw-daemon/");
    return {
...
describe("gatherDaemonStatus", () => {
  ...
  beforeEach(() => {
    ...
    process.env.OPENCLAW_STATE_DIR = "/tmp/mioclaw-cli";
    process.env.OPENCLAW_CONFIG_PATH = "/tmp/mioclaw-cli/mioclaw.json";
