...
${formatHelpExamples([
  ['mioclaw agent --to +15555550123 --message "status update"', "Start a new session."],
  ['mioclaw agent --agent ops --message "Summarize logs"', "Use a specific agent."],
  [
    'mioclaw agent --session-id 1234 --message "Summarize inbox" --thinking medium',
    "Target a session with explicit thinking level.",
  ],
  [
    'mioclaw agent --to +15555550123 --message "Trace logs" --verbose on --json',
    "Enable verbose logging and JSON output.",
  ],
  ['mioclaw agent --to +15555550123 --message "Summon reply" --deliver', "Deliver reply."],
  [
    'mioclaw agent --agent ops --message "Generate report" --deliver --reply-channel slack --reply-to "#reports"',
    "Send reply to a different channel/target.",
  ],
])}

${theme.muted("Docs:")} ${formatDocsLink("/cli/agent", "docs.mioclaw.ai/cli/agent")}`,
    )
    .action(async (opts) => {
  ...
  const agents = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/agents", "docs.mioclaw.ai/cli/agents")}\n`,
    );
...
${theme.heading("Examples:")}
${formatHelpExamples([
  ['mioclaw agents set-identity --agent main --name "Mioclaw" --emoji "🦞"', "Set name + emoji."],
  ["mioclaw agents set-identity --agent main --avatar avatars/mioclaw.png", "Set avatar path."],
  [
    "mioclaw agents set-identity --workspace ~/.mioclaw/workspace --from-identity",
    "Load from IDENTITY.md.",
  ],
  [
    "mioclaw agents set-identity --identity-file ~/.mioclaw/workspace/IDENTITY.md --agent main",
