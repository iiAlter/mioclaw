...
function isCompletionProfileHeader(line: string): boolean {
  return line.trim() === "# Mioclaw Completion";
}
...
function isSlowDynamicCompletionLine(line: string, binName: string): boolean {
  // Matches patterns like: source <(mioclaw completion --shell zsh)
  return (
...
function updateCompletionProfile(
  ...
  const block = `# Mioclaw Completion\n${sourceLine}`;
 ...
 * Returns true if profile has `source <(mioclaw completion ...)` instead of cached file.
 */
...
export function registerCompletionCli(program: Command) {
  program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/completion", "docs.mioclaw.ai/cli/completion")}\n`,
    )
    .addOption(
...
export async function installCompletion(shell: string, yes: boolean, binName = "mioclaw") {
  const home = process.env.HOME || os.homedir();
  let profilePath = "";
  let sourceLine = "";
...
function generateFishCompletion(program: Command): string {
  ...
  const visit = (cmd: Command, parents: string[]) => {
    ...
    // For root: complete -c mioclaw -n "__fish_use_subcommand" -a "subcmd" -d "desc"
    ...
    if (parents.length === 0) {
      ...
      // To properly scope to 'mioclaw gateway' and not 'mioclaw other gateway', we need to check the sequence.
