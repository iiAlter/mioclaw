...
describe("npm-resolution helpers", () => {
  it("keeps original spec when pin is disabled", () => {
    const result = resolvePinnedNpmSpec({
      rawSpec: "@mioclaw/plugin-alpha@latest",
      pin: false,
      resolvedSpec: "@mioclaw/plugin-alpha@1.2.3",
    });
    expect(result).toEqual({
      recordSpec: "@mioclaw/plugin-alpha@latest",
    });
  });
  ...
  it("returns pinned spec notice when resolved spec is available", () => {
    ...
    expect(result).toEqual({
      recordSpec: "@mioclaw/plugin-alpha@1.2.3",
      pinNotice: "Pinned npm install record to @mioclaw/plugin-alpha@1.2.3.",
    });
  });
  ...
  it("maps npm resolution metadata to install fields", () => {
    expect(
      ...
      resolvedName: "@mioclaw/plugin-alpha",
  ...
  it("builds common npm install record fields", () => {
    expect(
      buildNpmInstallRecordFields({
        spec: "@mioclaw/plugin-alpha@1.2.3",
        installPath: "/tmp/mioclaw/extensions/alpha",
        version: "1.2.3",
        resolution: {
          name: "@mioclaw/plugin-alpha",
          version: "1.2.3",
  ...
  it("resolves pinned install record and emits pin notice", () => {
    ...
    expect(logs).toEqual(["Pinned npm install record to @mioclaw/plugin-alpha@1.2.3."]);
  ...
  it("resolves pinned install record for CLI and formats warning output", () => {
    ...
    const record = resolvePinnedNpmInstallRecordForCli(
      "@mioclaw/plugin-alpha@latest",
      true,
      "/tmp/mioclaw/extensions/alpha",
      "1.2.3",
      undefined,
      (message) => logs.push(message),
    ...
    expect(record).toEqual({
      source: "npm",
      spec: "@mioclaw/plugin-alpha@latest",
      installPath: "/tmp/mioclaw/extensions/alpha",
      version: "1.2.3",
