    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["mioclaw memory status", "Show index and provider status."],
          ["mioclaw memory status --deep", "Probe embedding provider readiness."],
          ["mioclaw memory index --force", "Force a full reindex."],
          ['mioclaw memory search "meeting notes"', "Quick search using positional query."],
          [
            'mioclaw memory search --query "deployment" --max-results 20',
            "Limit results for focused troubleshooting.",
          ],
          ["mioclaw memory status --json", "Output machine-readable JSON (good for scripts)."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/memory", "docs.mioclaw.ai/cli/memory")}\n`,
    );
