    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["mioclaw nodes status", "List known nodes with live status."],
          ["mioclaw nodes pairing pending", "Show pending node pairing requests."],
          ['mioclaw nodes run --node <id> --raw "uname -a"', "Run a shell command on a node."],
          ["mioclaw nodes camera snap --node <id>", "Capture a photo from a node camera."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/nodes", "docs.mioclaw.ai/cli/nodes")}\n`,
    );
