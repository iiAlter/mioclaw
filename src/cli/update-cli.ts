...
export function registerUpdateCli(program: Command) {
  const update = program
    .command("update")
    .description("Update Mioclaw and inspect update channel status")
    ...
    .addHelpText("after", () => {
      const examples = [
        ["mioclaw update", "Update a source checkout (git)"],
        ["mioclaw update --channel beta", "Switch to beta channel (git + npm)"],
        ["mioclaw update --channel dev", "Switch to dev channel (git + npm)"],
        ["mioclaw update --tag beta", "One-off update to a dist-tag or version"],
        ["mioclaw update --dry-run", "Preview actions without changing anything"],
        ["mioclaw update --no-restart", "Update without restarting the service"],
        ["mioclaw update --json", "Output result as JSON"],
        ["mioclaw update --yes", "Non-interactive (accept downgrade prompts)"],
        ["mioclaw update wizard", "Interactive update wizard"],
        ["mioclaw --update", "Shorthand for mioclaw update"],
      ] as const;
...
${theme.heading("Switch channels:")}
  ...
  - Run mioclaw update status to see the active channel and source
...
${theme.muted("Docs:")} ${formatDocsLink("/cli/update", "docs.mioclaw.ai/cli/update")}`;
    })
    .action(async (opts) => {
      try {
        await updateCommand({
  ...
  update
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["mioclaw update status", "Show channel + version status."],
          ["mioclaw update status --json", "JSON output."],
          ["mioclaw update status --timeout 10", "Custom timeout."],
        ])}\n\n${theme.heading("Notes:")}\n${theme.muted(
          "- Shows current update channel (stable/beta/dev) and source",
        )}\n${theme.muted("- Includes git tag/branch/SHA for source checkouts")}\n\n${theme.muted(
          "Docs:",
        )} ${formatDocsLink("/cli/update", "docs.mioclaw.ai/cli/update")}`,
    )
    .action(async (opts, command) => {
      try {
