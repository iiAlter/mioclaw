...
function auditGatewayToken(
  ...
  issues.push({
    ...
    detail: "Run `mioclaw gateway install --force` to remove embedded service token.",
    level: "recommended",
  });
  ...
  issues.push({
    ...
    message:
      "Gateway service OPENCLAW_GATEWAY_TOKEN does not match gateway.auth.token in mioclaw.json",
...
export function checkTokenDrift(params: {
  ...
  if (configToken && serviceToken !== configToken) {
    return {
      ...
      detail: "Run `mioclaw gateway install --force` to sync the token.",
      level: "recommended",
    };
  }
