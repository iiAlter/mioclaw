...
describe("browser config", () => {
  it("defaults to enabled with loopback defaults and lobster-orange color", () => {
    ...
    expect(profile?.name).toBe("mioclaw");
    expect(profile?.driver).toBe("mioclaw");
    expect(profile?.cdpPort).toBe(18800);
    expect(profile?.cdpUrl).toBe("http://127.0.0.1:18800");

    const mioclaw = resolveProfile(resolved, "mioclaw");
    expect(mioclaw?.driver).toBe("mioclaw");
    expect(mioclaw?.cdpPort).toBe(18800);
    expect(mioclaw?.cdpUrl).toBe("http://127.0.0.1:18800");
  ...
  it("derives default ports from OPENCLAW_GATEWAY_PORT when unset", () => {
    withEnv({ OPENCLAW_GATEWAY_PORT: "19001" }, () => {
      ...
      const mioclaw = resolveProfile(resolved, "mioclaw");
      expect(mioclaw?.cdpPort).toBe(19012);
      expect(mioclaw?.cdpUrl).toBe("http://127.0.0.1:19012");
    });
  });
  ...
  it("derives default ports from gateway.port when env is unset", () => {
    withEnv({ OPENCLAW_GATEWAY_PORT: undefined }, () => {
      ...
      const mioclaw = resolveProfile(resolved, "mioclaw");
      expect(mioclaw?.cdpPort).toBe(19022);
      expect(mioclaw?.cdpUrl).toBe("http://127.0.0.1:19022");
    });
  });
  ...
  it("supports overriding the local CDP auto-allocation range start", () => {
    ...
    expect(mioclaw?.cdpPort).toBe(19000);
    expect(mioclaw?.cdpUrl).toBe("http://127.0.0.1:19000");
  });
  ...
  it("treats non-loopback cdpUrl as remote", () => {
    ...
    const profile = resolveProfile(resolved, "mioclaw");
  ...
  it("does not add the built-in chrome extension profile if the derived relay port is already used", () => {
    const resolved = resolveBrowserConfig({
      profiles: {
        mioclaw: { cdpPort: 18792, color: "#FF4500" },
      },
    });
    expect(resolveProfile(resolved, "chrome")).toBe(null);
    expect(resolved.defaultProfile).toBe("mioclaw");
  });
  ...
  describe("default profile preference", () => {
    it("defaults to mioclaw profile when defaultProfile is not configured", () => {
      const resolved = resolveBrowserConfig({
        headless: false,
        noSandbox: false,
      });
    ...
    it("keeps mioclaw default when headless=true", () => {
      const resolved = resolveBrowserConfig({
        headless: true,
      });
      expect(resolved.defaultProfile).toBe("mioclaw");
    ...
    it("keeps mioclaw default when noSandbox=true", () => {
      const resolved = resolveBrowserConfig({
        noSandbox: true,
      });
      expect(resolved.defaultProfile).toBe("mioclaw");
    ...
    it("keeps mioclaw default when both headless and noSandbox are true", () => {
      const resolved = resolveBrowserConfig({
        headless: true,
        noSandbox: true,
      });
