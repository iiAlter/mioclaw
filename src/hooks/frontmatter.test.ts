...
`;
    ...
    expect(parsed.mioclaw.emoji).toBe("💾");
    expect(parsed.mioclaw.events).toEqual(["command:new"]);
  });
...
`;
    ...
    const parsed = JSON.parse(result.metadata);
    expect(parsed.mioclaw.emoji).toBe("📝");
    expect(parsed.mioclaw.events).toEqual(["command"]);
    expect(parsed.mioclaw.requires.config).toEqual(["workspace.dir"]);
    expect(parsed.mioclaw.install[0].kind).toBe("bundled");
  });

  it("handles single-line metadata (inline JSON)", () => {
    const content = `---
name: simple-hook
metadata: {"mioclaw": {"events": ["test"]}}
---
`;
    const result = parseFrontmatter(content);
    expect(result.name).toBe("simple-hook");
    expect(result.metadata).toBe('{"mioclaw": {"events": ["test"]}}');
  });
...
describe("resolveOpenClawMetadata", () => {
  it("extracts mioclaw metadata from parsed frontmatter", () => {
    const frontmatter = {
      name: "test-hook",
  ...
  it("returns undefined when mioclaw key is missing", () => {
    const frontmatter = {
  ...
  it("handles install specs", () => {
    const frontmatter = {
      metadata: JSON.stringify({
        mioclaw: {
          events: ["command"],
          install: [
            { id: "bundled", kind: "bundled", label: "Bundled with Mioclaw" },
            { id: "npm", kind: "npm", package: "@mioclaw/hook" },
          ],
        },
      }),
    };
    ...
    expect(result?.install?.[1].package).toBe("@mioclaw/hook");
  });
    ...
    "mioclaw":
      {
        ...
        "events": ["command:new", "command:reset"],
        "requires": { "config": ["workspace.dir"] },
        "install": [{ "id": "bundled", "kind": "bundled", "label": "Bundled with Mioclaw" }],
      },
  }
---
...
`;
    ...
    const mioclaw = resolveOpenClawMetadata(frontmatter);
    expect(mioclaw).toBeDefined();
    expect(mioclaw?.emoji).toBe("💾");
    expect(mioclaw?.events).toEqual(["command:new", "command:reset"]);
    expect(mioclaw?.requires?.config).toEqual(["workspace.dir"]);
    expect(mioclaw?.install?.[0].kind).toBe("bundled");
  });
...
`;
    const frontmatter = parseFrontmatter(content);
    const mioclaw = resolveOpenClawMetadata(frontmatter);
    expect(mioclaw?.emoji).toBe("disk");
    expect(mioclaw?.events).toEqual(["command:new"]);
  });
});
