...
describe("createPiToolsSandboxContext", () => {
  it("provides stable defaults for pi-tools sandbox tests", () => {
    ...
    expect(sandbox.containerName).toBe("mioclaw-sbx-test");
    expect(sandbox.containerWorkdir).toBe("/workspace");
    expect(sandbox.docker.image).toBe("mioclaw-sandbox:bookworm-slim");
    expect(sandbox.docker.containerPrefix).toBe("mioclaw-sbx-");
