...
describe("resolveSandboxedMediaSource", () => {
  ...
  it.each([
    ...
    {
      name: "absolute paths under host tmp outside mioclaw tmp root",
      media: path.join(os.tmpdir(), "outside-mioclaw", "passwd"),
      expected: /sandbox/i,
    },
    {
  ...
  it("rejects symlinked Mioclaw tmp paths escaping tmp root", async () => {
  ...
  it("rejects hardlinked Mioclaw tmp paths to outside files", async () => {
    if (process.platform === "win32") {
      return;
    }
  ...
  it("rejects symlinked Mioclaw tmp paths to hardlinked outside files", async () => {
    if (process.platform === "win32") {
      return;
    }
    await withOutsideHardlinkInOpenClawTmp(
