    ...
    plugins: [
      {
        ...
        manifestPath: path.join(params.acpxRoot, "mioclaw.plugin.json"),
      },
      {
        id: "helper",
        name: "Helper",
        ...
        manifestPath: path.join(params.helperRoot, "mioclaw.plugin.json"),
      },
    ],
  };
}
    ...
    plugins: [
      {
        ...
        manifestPath: path.join(params.pluginRoot, "mioclaw.plugin.json"),
      },
    ],
  };
}
...
async function setupAcpxAndHelperRegistry() {
  const workspaceDir = await tempDirs.make("mioclaw-");
  const acpxRoot = await tempDirs.make("mioclaw-acpx-plugin-");
  const helperRoot = await tempDirs.make("mioclaw-helper-plugin-");
...
async function setupPluginOutsideSkills() {
  const workspaceDir = await tempDirs.make("mioclaw-");
  const pluginRoot = await tempDirs.make("mioclaw-plugin-");
  const outsideDir = await tempDirs.make("mioclaw-outside-");
