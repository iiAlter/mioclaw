...
export function buildSandboxCreateArgs(params: {
  ...
  args.push("--label", "mioclaw.sandbox=1");
  args.push("--label", `mioclaw.sessionKey=${params.scopeKey}`);
  args.push("--label", `mioclaw.createdAtMs=${createdAtMs}`);
  if (params.configHash) {
    args.push("--label", `mioclaw.configHash=${params.configHash}`);
  }
...
async function readContainerConfigHash(containerName: string): Promise<string | null> {
  return await readDockerContainerLabel(containerName, "mioclaw.configHash");
}

function formatSandboxRecreateHint(params: { scope: SandboxConfig["scope"]; sessionKey: string }) {
  if (params.scope === "session") {
    return formatCliCommand(`mioclaw sandbox recreate --session ${params.sessionKey}`);
  }
  if (params.scope === "agent") {
    const agentId = resolveSandboxAgentId(params.sessionKey) ?? "main";
    return formatCliCommand(`mioclaw sandbox recreate --agent ${agentId}`);
  }
  return formatCliCommand("mioclaw sandbox recreate --all");
}
