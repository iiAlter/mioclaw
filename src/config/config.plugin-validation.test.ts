...
async function writePluginFixture(params: {
  ...
  await fs.writeFile(
    path.join(params.dir, "mioclaw.plugin.json"),
...
describe("config plugin validation", () => {
  ...
  const suiteEnv = () =>
    ({
      ...
      OPENCLAW_STATE_DIR: path.join(suiteHome, ".mioclaw"),
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "mioclaw-config-plugin-validation-"));
