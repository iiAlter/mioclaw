...
describe("sandbox browser binds config", () => {
  ...
  it("defaults browser network to dedicated sandbox network", () => {
    ...
    expect(resolved.network).toBe("mioclaw-sandbox-browser");
  });

  it("prefers agent browser network over global browser network", () => {
    const resolved = resolveSandboxBrowserConfig({
      scope: "agent",
      globalBrowser: { network: "mioclaw-sandbox-browser-global" },
      agentBrowser: { network: "mioclaw-sandbox-browser-agent" },
    });
    expect(resolved.network).toBe("mioclaw-sandbox-browser-agent");
  });
