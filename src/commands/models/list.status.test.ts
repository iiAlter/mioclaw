...
const mocks = vi.hoisted(() => {
  ...
  return {
    store,
    resolveOpenClawAgentDir: vi.fn().mockReturnValue("/tmp/mioclaw-agent"),
    resolveAgentDir: vi.fn().mockReturnValue("/tmp/mioclaw-agent"),
    ...
    createConfigIO: vi.fn().mockReturnValue({
      configPath: "/tmp/mioclaw-dev/mioclaw.json",
    }),
...
async function withAgentScopeOverrides<T>(
  ...
  try {
    ...
    if (originalAgentDir) {
      ...
      mocks.resolveAgentDir.mockReturnValue("/tmp/mioclaw-agent");
    }
  }
}
...
describe("modelsStatusCommand auth overview", () => {
  it("includes masked auth sources in JSON output", async () => {
    ...
    expect(payload.configPath).toBe("/tmp/mioclaw-dev/mioclaw.json");
    expect(payload.auth.storePath).toBe("/tmp/mioclaw-agent/auth-profiles.json");
    ...
    await withAgentScopeOverrides(
      {
        ...
        agentDir: "/tmp/mioclaw-agent-custom",
      },
      async () => {
        ...
        expect(payload.agentDir).toBe("/tmp/mioclaw-agent-custom");
