...
export async function statusCommand(
  ...
  runtime.log(theme.heading("Mioclaw status"));
  runtime.log("");
  ...
  if (pairingRecovery) {
    ...
    if (pairingRecovery.requestId) {
      runtime.log(
        theme.muted(
          `Recovery: ${formatCliCommand(`mioclaw devices approve ${pairingRecovery.requestId}`)}`,
        ),
      );
    }
    runtime.log(theme.muted(`Fallback: ${formatCliCommand("mioclaw devices approve --latest")}`));
    runtime.log(theme.muted(`Inspect: ${formatCliCommand("mioclaw devices list")}`));
  }
  ...
  runtime.log(theme.muted(`Full report: ${formatCliCommand("mioclaw security audit")}`));
  runtime.log(theme.muted(`Deep probe: ${formatCliCommand("mioclaw security audit --deep")}`));
  ...
  runtime.log(`  Need to share?      ${formatCliCommand("mioclaw status --all")}`);
  runtime.log(`  Need to debug live? ${formatCliCommand("mioclaw logs --follow")}`);
  if (gatewayReachable) {
    runtime.log(`  Need to test channels? ${formatCliCommand("mioclaw status --deep")}`);
  } else {
    runtime.log(`  Fix reachability first: ${formatCliCommand("mioclaw gateway probe")}`);
  }
}
