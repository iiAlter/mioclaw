...
export async function channelsCapabilitiesCommand(
  ...
  if (opts.account && (!rawChannel || rawChannel === "all")) {
    runtime.log("Error: --account requires a specific --channel.");
    runtime.exit(1);
    return;
  }
  ...
  if (!selected || selected.length === 0) {
    runtime.log(`Error: Unknown channel "${rawChannel}".`);
    runtime.exit(1);
    return;
  }
  ...
  for (const report of reports) {
    ...
    runtime.log(label);
    runtime.log(`Support: ${formatSupport(report.support)}`);
    if (report.actions && report.actions.length > 0) {
      runtime.log(`Actions: ${report.actions.join(", ")}`);
    }
    if (report.configured === false || report.enabled === false) {
      ...
      runtime.log(`Status: ${configuredLabel}, ${enabledLabel}`);
    }
    runtime.log("");
  }
}
